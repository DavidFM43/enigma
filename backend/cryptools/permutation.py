...
def encrypt(plain_text: str, key: list[int]) -> str:
    ...
    if len(plain_text) % m != 0: return 'El texto no se puede partir'
    ...
    def chunks( text: str, m: int ) -> list[list[str]]:
        ...
        for i in plain_text:
            buffer.append(i)
            c+= 1
            if c == m: pieces.append(buffer); buffer =list(); c=0
        return pieces
...
def decrypt(cipher_text: str, key: list[int]) ->str:
    ...
    #Validación para partir el texto
    if len(cipher_text) % m != 0: return 'El texto no se puede partir'
...
if __name__ == "__main__":
    ...
    """
    assert encrypt('shesellsseashellsbytheseashore', [3,6,1,5,2,4]) == 'EESLSHSALSESLSHBLEHSYEETHRAEOS'
    assert decrypt('EESLSHSALSESLSHBLEHSYEETHRAEOS', [3,6,1,5,2,4]) == 'shesellsseashellsbytheseashore'
