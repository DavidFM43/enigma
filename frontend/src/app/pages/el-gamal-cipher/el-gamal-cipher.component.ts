...
export class ElGamalCipherComponent implements OnInit {
  ...
  constructor(private connection: ConnectionService) {
    this.arguments = new UntypedFormGroup({
      P: new UntypedFormControl("", [
        Validators.pattern(/^[0-9]\d*$/),
        Validators.required,
      ]),
      G: new UntypedFormControl("", [
        Validators.pattern(/^[0-9]\d*$/),
        Validators.required,
      ]),
      H: new UntypedFormControl("", [
        Validators.pattern(/^[0-9]\d*$/),
        Validators.required,
      ]),
  ...
  random(): void {
    ...
    this.connection.ElGamalGetKeys().subscribe((ansKeys: ElGamalKeys) => {
      console.log(ansKeys);
  ...
  encrypt(): void {
    ...
    this.connection
      .ElGamalEncrypt(publicKey, this.arguments.get("plainText").value.replace(/ /g, ""))
  ...
  decrypt(): boolean {
    ...
    let arrNums: string[] = this.arguments.get("plainText").value.replace(/ /g, "").split(",");
