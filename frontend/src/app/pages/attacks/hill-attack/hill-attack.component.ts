...
export class HillAttackComponent implements OnInit {
    ...
    this.arguments = new FormGroup(
      {
        plainText: new FormControl('', [Validators.required,
          ...
          cipherText: new FormControl('', [Validators.required,
            Validators.pattern('^[a-zA-Z ]+[ ]*[a-zA-Z ]*$')]),
            keySize: new FormControl('', [Validators.required, Validators.pattern('^[1-9]*$'), 
            Validators.min(2), Validators.max(4)]),
      }
    )
    this.options = null;
  ...
  attack():void{
    ...
    let normalizedTextCipher: string =  this.normalizer
    .setplainText(this.arguments.get('cipherText').value);
    this.connection.hillAttack(normalizedTextCipher ,normalizedText, parseInt(this.keySize.value)).subscribe((ans:hillAttackerResponse) => {
      this.options = ans;
      if (!ans.error) {
        this.error = false;
      }else{
